...
Kaynak: Supabase `iiagqsbiexyukupkffwb` projesindeki `workflow_definitions`, `workflow_steps`, `positions`, `employee_positions`, `employees` tabloları. Oluşturulma tarihi: 2026-05-03. Tüm listelenen workflow tanımları aktiftir.
## Notlar
...
- `Aktif atama yok`: İlgili statik pozisyona bugün itibarıyla aktif/primary çalışan bulunamadı.
## Özet
...
|---:|---|---|---|---|
| 1 | Talep Eden | `REQUESTER` | Talep/form oluşturan kullanıcı | Dinamik |
| 2 | Personel Müdürlüğü | `STATIC_POSITION` | İnsan Kaynakları Uzmanı (`IK300`) | Aktif atama yok |
| 3 | Muhasebe Müdürlüğü | `STATIC_POSITION` | Muhasebe Şefi (`M200`) | Aktif atama yok |
...
| 6 | Yönetim Kurulu Başkanı | `STATIC_POSITION` | Yönetim Kurulu Başkanı (`YKB`) | Admin Test |
